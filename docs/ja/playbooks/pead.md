...
- **決算当日の飛び乗りとして使わない。** このプレイブックはギャップ当日には決してエントリーしません。まず赤週足が形成され、その後にブレイクアウトが必要です。プルバックせずにそのまま上昇する「gap-and-go」は明示的に `pead-screener` の候補ではありません（赤週足が無ければリスクを定義できないためです）。
...
ここで `--min-gap 3.0` を渡すことが重要です。これは `abs(gap_pct)`（`analyze_earnings_trades.py`）による大きさのフィルタなので**明示的に渡してください**。実際、このパイプライン全体でギャップの大きさに下限を課しているのはここだけであり、それでも方向はチェックしません。
...
| Stage | 意味 | アクション |
|---|---|---|
| `MONITORING` | 窓内でギャップアップしたが赤週足はまだ無い | ウォッチリストへ。週次で赤キャンドルの形成を確認 |
...
このアダプタはスクリーナーの実際のフィールド名である `stage` と `stop_price` を読みます。実際のレコードには存在しない `status`/`stop_loss` ではありません。`thesis_type` はすべての PEAD 登録で `earnings_drift` に固定されます。Momentum Burst の `growth_momentum` とは異なり、この値は他のどのアダプタとも共有されません。
...
| 保有期間 | 2〜6週間 | 2〜5セッション |
| `thesis_type` | `earnings_drift`（専用アダプタ） | `growth_momentum`（CANSLIMと共有） |
...
- このパイプラインにはまだ `workflows/*.yaml` の manifest が存在しないため、Trading Skills Navigator によるルーティングは追加されるまでスコープ外です。
