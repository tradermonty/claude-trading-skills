...
スクリーナーから出力されたシグナルをランキング・サイジング・ブラケット注文発注・日中リスク監視・引け後リコンサイルまで自動化するスタック。すべて決定論的でユニットテスト済み。デフォルトは Alpaca ペーパー口座。

| スキル | 説明 | API要件 |
|-------|------|--------|
| **[Trade Loop Orchestrator]({{ '/ja/skills/trade-loop-orchestrator/' | relative_url }})** | 5分ごとに動く司令塔。アダプタ経由でスクリーナー出力を読み込み、レジームゲート適用、シグナル統合・重複排除、ポートフォリオ制約を加味したサイジング、Alpaca Executor経由のブラケット注文発注を実行 | <span class="badge badge-api">Alpaca必須</span> |
| **[Alpaca Executor]({{ '/ja/skills/alpaca-executor/' | relative_url }})** | Alpaca REST薄ラッパー。冪等キー付きブラケット注文、キルスイッチ向けポートフォリオ全強制決済、構造化監査ログを提供 | <span class="badge badge-api">Alpaca必須</span> |
| **[Kill Switch]({{ '/ja/skills/kill-switch/' | relative_url }})** | trading_params.yamlの日次損失・ポジション数・セクター比率上限を2分ごとに監視するウォッチドッグ。ハード違反で全強制決済、ソフト違反で新規エントリーをブロック | <span class="badge badge-api">Alpaca必須</span> |
| **[EOD Reconciliation]({{ '/ja/skills/eod-reconciliation/' | relative_url }})** | 16:30 ET夜間ジョブ。ループ判断とAlpaca約定を突合し、トレード別P&Lを按分、該当セシスをクローズしtrader-memory-coreポストモーテムを起動 | <span class="badge badge-api">Alpaca必須</span> |
| **[Paper Replay Harness]({{ '/ja/skills/paper-replay-harness/' | relative_url }})** | CSV OHLCVバーに対する決定論的ヒストリカルシミュレータ。同一入力→ビット単位で同一出力。SimBrokerは保守側（同バーで両ヒット時はストップ優先）。エクイティカーブ、ドローダウン、シャープ、Rマルチプル統計を出力 | <span class="badge badge-free">API不要</span> |
| **[Macro Indicator Dashboard]({{ '/ja/skills/macro-indicator-dashboard/' | relative_url }})** | 厳選FRED系列を取得しレジームスコア（RISK_ON / NEUTRAL / RISK_OFF）を算出、トレードループが各イテレーションで参照するダッシュボードを生成 | <span class="badge badge-api">FRED必須</span> |
| **[Relative Strength Momentum Scanner]({{ '/ja/skills/relative-strength-momentum-scanner/' | relative_url }})** | IBD流RSコンポジット（3/6/9/12ヶ月加重）を1〜99パーセンタイルで順位付け、トレンドフィルタ（MA50>MA200、52週高値の10%以内）と MA20押し目トリガー。entry_ready / watchlist / filtered の3状態でループに供給 | <span class="badge badge-free">API不要</span> |
...
| Trade Loop Orchestrator | - | - | 必須 |
| Alpaca Executor | - | - | 必須 |
| Kill Switch | - | - | 必須 |
| EOD Reconciliation | - | - | 必須 |
...
- **FRED API**（無料, https://fred.stlouisfed.org/docs/api/api_key.html）-- Macro Indicator Dashboard と Macro Regime Detector で必須。
