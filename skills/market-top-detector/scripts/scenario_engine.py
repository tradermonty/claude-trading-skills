...
def generate_scenarios(component_scores: Dict[str, float],
                       data_availability: Dict[str, bool] = None) -> List[Dict]:
    """
    ...
    scenarios.append({
        ...
        "delta": round(s1_result["composite_score"] - _current_score(component_scores, data_availability), 1),
    })
    ...
    scenarios.append({
        ...
        "delta": round(s2_result["composite_score"] - _current_score(component_scores, data_availability), 1),
    })
    ...
    scenarios.append({
        ...
        "delta": round(s3_result["composite_score"] - _current_score(component_scores, data_availability), 1),
    })
    ...
    scenarios.append({
        ...
        "delta": round(s4_result["composite_score"] - _current_score(component_scores, data_availability), 1),
    })

    return scenarios


def _current_score(component_scores: Dict[str, float],
                   data_availability: Dict[str, bool] = None) -> float:
