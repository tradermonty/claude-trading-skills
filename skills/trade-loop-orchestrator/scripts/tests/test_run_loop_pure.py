...
def test_size_position_normal(profile):
    qty, detail = rl.size_position(entry=100, stop=95, profile=profile,
                                   exposure_scale=1.0)
...
def test_size_position_constrained_by_risk(profile):
    qty, detail = rl.size_position(entry=100, stop=98, profile=profile,
                                   exposure_scale=1.0)
...
def test_size_position_scales_down_with_exposure(profile):
    qty_full, _ = rl.size_position(entry=100, stop=90, profile=profile,
                                   exposure_scale=1.0)
    qty_half, _ = rl.size_position(entry=100, stop=90, profile=profile,
                                   exposure_scale=0.5)
...
def test_size_position_floors_at_quarter(profile):
    ...
    qty, detail = rl.size_position(entry=100, stop=90, profile=profile,
                                   exposure_scale=0.0)
    assert detail["scaled_max_risk"] == pytest.approx(profile["account_size_usd"]
...
def test_size_position_zero_when_below_min(profile):
    ...
    qty, detail = rl.size_position(entry=100, stop=95, profile=small,
                                   exposure_scale=1.0)
    assert qty == 0
...
def test_size_position_invalid_prices(profile):
    qty, detail = rl.size_position(entry=0, stop=0, profile=profile,
                                   exposure_scale=1.0)
    assert qty == 0


def test_size_position_entry_equals_stop(profile):
    qty, _ = rl.size_position(entry=100, stop=100, profile=profile,
                              exposure_scale=1.0)
    assert qty == 0
...
def test_sector_cap_clear():
    assert not rl.would_breach_sector_cap(
        "AAPL", "Technology", new_notional=5000,
        current_exposures={"Technology": 10000},
        account_equity=100000, cap_pct=25.0,
    )  # projected 15% < 25%


def test_sector_cap_breached():
    assert rl.would_breach_sector_cap(
        "AAPL", "Technology", new_notional=20000,
        current_exposures={"Technology": 10000},
        account_equity=100000, cap_pct=25.0,
    )  # projected 30% > 25%
...
def test_sector_cap_first_entry_in_sector():
    assert not rl.would_breach_sector_cap(
        "JPM", "Financials", new_notional=20000,
        current_exposures={}, account_equity=100000, cap_pct=25.0,
    )
...
def test_in_trading_window_blackout():
    ...
    cfg = {"global": {
        "trading_hours": {"start": "09:45", "end": "15:45",
...
def test_in_trading_window_weekend_excluded():
    ...
    cfg = {"global": {"trading_hours": {
        "start": "09:45", "end": "15:45", "timezone": "America/New_York"}}}
    inside, reason = rl.in_trading_window(cfg)
    assert not inside
    assert "weekend" in reason
